...
export default async function AnnouncementsPage() {
  ...
  const { announcements } = await getAnnouncementsData(supabase)
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card className="p-4">
          ...
          <p className="text-2xl font-bold mt-1">{announcementsList.length}</p>
        </Card>
        <Card className="p-4">
          <p className="text-sm text-gray-600">Active Now</p>
          <p className="text-2xl font-bold mt-1">
            {announcementsList.filter((a: any) => a.status === 'active').length}
          </p>
        </Card>
        <Card className="p-4">
          <p className="text-sm text-gray-600">Scheduled</p>
          <p className="text-2xl font-bold mt-1">
            {announcementsList.filter((a: any) => a.status === 'scheduled').length}
          </p>
        </Card>
        <Card className="p-4">
          <p className="text-sm text-gray-600">Drafts</p>
          <p className="text-2xl font-bold mt-1">
            {announcementsList.filter((a: any) => a.status === 'draft').length}
          </p>
        </Card>
      </div>
