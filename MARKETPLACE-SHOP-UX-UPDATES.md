...
**Changes:**
- **Server** ([app/marketplace/page.tsx](app/marketplace/page.tsx)): Profile fetch extended with `teaching_class_types`, `teaching_learner_paths`, `teaching_strand_ids`, `teaching_sped_level_ids`, `grade_levels_taught`, `subjects_taught`. Teaching "complete" uses the same rule as profile edit (at least one teaching preference set). When user is logged in and teaching is **not** complete, recommended products are left empty (no fallback to trending) so the empty state can show the profile prompt. Passes `teachingComplete` to ProductTabs.
